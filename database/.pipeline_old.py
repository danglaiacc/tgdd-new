...
# set sequence id auto increment
with conn:
    with conn.cursor() as cur:
        cur.execute("SELECT setval('series_id_seq', (SELECT MAX(id) from series));")
        cur.execute("SELECT setval('invoice_id_seq', (SELECT MAX(id) from invoice));")
